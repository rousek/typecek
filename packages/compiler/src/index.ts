...
export interface CompileOptions {
  ...
  templatePath?: string;
  /** Absolute path to the output file, used to adjust relative import paths */
  outputPath?: string;
...
function adjustImportPath(rawPath: string, templatePath: string, outputPath: string): string {
  const absTarget = path.resolve(path.dirname(templatePath), rawPath);
  let rel = path.relative(path.dirname(outputPath), absTarget).split(path.sep).join("/");
  if (!rel.startsWith(".")) {
    rel = "./" + rel;
  }
  return rel;
...
export function compile(options: CompileOptions): CompileResult {
  ...
  if (typeName && from) {
    const emitFrom = options.templatePath && options.outputPath
      ? adjustImportPath(from, options.templatePath, options.outputPath)
      : from;
    code += `import type { ${typeName} } from "${emitFrom}";\n`;
  }
